...
async function getSubdivisions (req, res) {
  const [err, subdivisions] = await get(models.Subdivision.findAll({
    /* where:{estado: 'A'}, */
    include: [{ all: true }]
  }))
...
async function getSubdivision (req, res) {
  const [err, subdivision] = await get(models.Subdivision.findOne({
...
async function createSubdivision (req, res) {
  const [err, subdivision] = await get(models.Subdivision.create({
...
async function updateSubdivision (req, res) {
  const [err, subdivision] = await get(models.Subdivision.update({
