...
async function getAsistenciaFechaTurnosCount(req, res) {
  let [err, AsistenciaFechaTurnos] = await get(models.AsistenciaFechaTurno.count({
    where: { estado: 'A' },
  }))
  if (err) {
...
async function getAsistenciaFechaTurno(req, res) {
  let [err, AsistenciaFechaTurno] = await get(models.AsistenciaFechaTurno.findOne({
...
async function createAsistenciaFechaTurno(req, res) {
  let [err, asistenciaFechaTurno] = await get(models.AsistenciaFechaTurno.create({
...
async function createAllAsistenciaFechaTurno(req, res) {
  try {
    const t = await models.sequelize.transaction(async (t) => {
      ...
      let detalles = []
      for (let d of req.body.detalles) {
        console.log(d);
        detalles.push(
          {
      ...
      let [err, asistenciaRegistroPersonal] = await get(models.AsistenciaRegistroPersonal.bulkCreate(detalles, { transaction: t }));
      if (err) {
...
async function updateAsistenciaFechaTurno(req, res) {
  let [err, asistenciaFechaTurno] = await get(models.AsistenciaFechaTurno.update({
    estado: req.body.estado,
...
async function uploadFileAsistenciaFechaTurno(req, res) {
  let [err, asistencia_fecha_turno] = await get(models.AsistenciaFechaTurno.update({
  ...
  if (asistencia_fecha_turno == null) {
    logger.error(`404 PUT uploadFileAsistenciaFechaTurno, AsistenciaFechaTurno nulos.`)
    return res.status(404).json({ message: `AsistenciaFechaTurno nulos` })
  }
  logger.info(`200 PUT uploadFileAsistenciaFechaTurno, ${asistencia_fecha_turno[1][0].dataValues} values.`)
  res.status(200).json(asistencia_fecha_turno[1][0].dataValues)
}
