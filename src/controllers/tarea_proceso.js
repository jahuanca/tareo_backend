...
async function getTareaProcesos (req, res) {
  const [err, tareaProcesos] = await get(models.TareaProceso.findAll({
    /* where: { estado: 'A' },
        include: [{ all: true }] */
  }))
...
async function getTareaProceso (req, res) {
  const [err, tareaProceso] = await get(models.TareaProceso.findOne({
...
async function createTareaProceso (req, res) {
  const [err, tareaProceso] = await get(models.TareaProceso.create({
...
async function updateTareaProceso (req, res) {
  const [err, tareaProceso] = await get(models.TareaProceso.update({
